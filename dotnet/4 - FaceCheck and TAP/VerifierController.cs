    ...
    public class VerifierController : Controller
    {
        ...
        public async Task<ActionResult> CreateTap( string id) {
            try {
                ...
                if (!cachedData.ContainsKey( "givenName" ) || !cachedData.ContainsKey( "surname" ) ) {
                    return BadRequest( new { error = "400", error_description = $"givenName/surname missing in cached data" } );
                }

                string givenName = cachedData["givenName"].ToString();
                string surname = cachedData["surname"].ToString();
                ...
                var cacheData = new {
                    ...
                    message = $"Welcome aboard, {givenName}.",
        ...
        public async Task<ActionResult> PresentationCallback()
        {
            try
            {
                ...
                {
                    string givenName = presentationResponse["verifiedCredentialsData"][0]["claims"]["givenName"].ToString();
                    string surname = presentationResponse["verifiedCredentialsData"][0]["claims"]["surname"].ToString();
                    
                    var cacheData = new {
                        status = "presentation_verified",
                        message = $"Presentation verified for {givenName} {surname}. Please wait for a minute for the system to validate details.",
                        givenName = givenName,
                        surname = surname
                    };
                    _log.LogTrace( $"{cacheData.status}, givenName={givenName}, surname={surname}" );
                    _cache.Set( state, JsonConvert.SerializeObject( cacheData ) );
                }
